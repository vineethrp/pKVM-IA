...
int kvm_emulate_rdmsr(struct kvm_vcpu *vcpu)
{
	...
	return __kvm_emulate_rdmsr(vcpu, kvm_ecx_read(vcpu), -1,
				   complete_fast_rdmsr);
#else
	return __kvm_emulate_rdmsr(vcpu, kvm_ecx_read(vcpu), -1, NULL);
#endif
}
...
{
	unsigned long nr = kvm_rax_read_raw(vcpu);
	int ret;

	switch (nr) {
	case PKVM_GHC_IOREAD: {
		unsigned int size = kvm_rcx_read_raw(vcpu);

		if (size > sizeof(*vcpu->arch.regs))
			goto invalid;

		/* Leverage SEV-ES MMIO emulation. */
		ret = kvm_sev_es_mmio(vcpu, false, kvm_rbx_read_raw(vcpu), size,
	...
	case PKVM_GHC_IOWRITE: {
		...
		/* Leverage SEV-ES MMIO emulation. */
		ret = kvm_sev_es_mmio(vcpu, true, kvm_rbx_read_raw(vcpu), size,
	...
	case PKVM_GHC_START_CPU:
		ret = pkvm_start_secondary_vcpu(vcpu->kvm, kvm_rbx_read_raw(vcpu));
		kvm_rax_write_raw(vcpu, ret);
		ret = 1;
		break;
	default:
