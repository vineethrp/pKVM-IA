					...
					struct kvm_vcpu *shared_vcpu)
{
	...
	case EXIT_REASON_IO_INSTRUCTION: {
		...
		if ((exit_qual & 8) != 0) {
			...
			unsigned long val = (size < 4) ? kvm_rax_read_raw(vcpu) : 0;
	...
	case EXIT_REASON_VMCALL:
		 ...
		 */
		if (kvm_rax_read_raw(vcpu) == PKVM_GHC_SHARE_MEM ||
		    kvm_rax_read_raw(vcpu) == PKVM_GHC_UNSHARE_MEM)
			break;
				       ...
				       struct kvm_vcpu *shared_vcpu)
{
	...
	case EXIT_REASON_IO_INSTRUCTION: {
		...
		if ((exit_qual & 8) == 0) { /* OUT */
			...
			shared_vcpu->arch.regs[VCPU_REGS_RAX] =
				kvm_rax_read_raw(vcpu) & GENMASK(size * 8 - 1, 0);
		}
		break;
	}
	case EXIT_REASON_MSR_WRITE:
	...
	case EXIT_REASON_VMCALL:
		 ...
		 */
		shared_vcpu->arch.regs[VCPU_REGS_RAX] = kvm_rax_read_raw(vcpu);
		shared_vcpu->arch.regs[VCPU_REGS_RCX] = kvm_rcx_read_raw(vcpu);
		shared_vcpu->arch.regs[VCPU_REGS_RDX] = kvm_rdx_read_raw(vcpu);
		shared_vcpu->arch.regs[VCPU_REGS_RBX] = kvm_rbx_read_raw(vcpu);
		shared_vcpu->arch.regs[VCPU_REGS_RSI] = kvm_rsi_read_raw(vcpu);
		break;
	default:
		break;
	}
...
static void pkvm_vmx_share_vcpu_state_with_host(struct kvm_vcpu *vcpu)
{
	...
	kvm_register_mark_available(shared_vcpu, VCPU_REG_EXIT_INFO_1);
	...
	kvm_register_mark_available(shared_vcpu, VCPU_REG_EXIT_INFO_2);
...
int pkvm_vmx_init(void)
{
	...
	enable_pml = false;
	/*
	 * Mode-based execute control makes the EPT XU bit (bit 10) govern
	 * user-mode instruction fetches separately from supervisor-mode ones.
	 * KVM's MMU sets both XS and XU (see shadow_xu_mask), but pKVM builds
	 * its own EPT entries in pkvm/vmx/ept.c and only ever sets
	 * VMX_EPT_EXECUTABLE_MASK, so with MBEC enabled every user-mode
	 * instruction fetch in a guest would EPT-violate.  pKVM has no use for
	 * the distinction either: its guest MMU is TDP-only, with no shadow
	 * paging and no user/supervisor split in its page-state model.  Keep
	 * MBEC off; it is enabled by default in KVM since Linux 7.2.
	 */
	enable_mbec = false;
	enable_vmware_backdoor = false;
