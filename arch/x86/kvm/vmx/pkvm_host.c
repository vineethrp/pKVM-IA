...
static void pkvm_cache_reg(struct kvm_vcpu *vcpu, enum kvm_reg reg)
{
	...
	case VCPU_REGS_RSP:
		...
		break;
	case VCPU_REG_RIP:
		vcpu->arch.rip = out.cache_reg.rip;
		break;
	case VCPU_REG_PDPTR:
		vcpu->arch.pdptrs[0] = out.cache_reg.pdptrs[0];
		vcpu->arch.pdptrs[1] = out.cache_reg.pdptrs[1];
		vcpu->arch.pdptrs[2] = out.cache_reg.pdptrs[2];
		vcpu->arch.pdptrs[3] = out.cache_reg.pdptrs[3];
		break;
	case VCPU_REG_CR0:
...
static void pkvm_flush_tlb_gva(struct kvm_vcpu *vcpu, gva_t addr, bool *full)
{
	if (!vcpu->arch.guest_state_protected)
...
static u32 pkvm_get_interrupt_shadow(struct kvm_vcpu *vcpu)
{
	...
	if (KVM_BUG_ON(pkvm_hypercall_out(get_interrupt_shadow, &out), vcpu->kvm))
		return 0;
...
static void pkvm_intel_pmu_mediated_load(struct kvm_vcpu *vcpu) {}
static void pkvm_intel_pmu_mediated_put(struct kvm_vcpu *vcpu) {}
...
static struct kvm_pmu_ops pkvm_host_vt_pmu_ops __initdata = {
	...
	.mediated_load = pkvm_intel_pmu_mediated_load,
	.mediated_put = pkvm_intel_pmu_mediated_put,
