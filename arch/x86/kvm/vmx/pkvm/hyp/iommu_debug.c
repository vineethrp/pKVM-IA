...
static inline void print_tbl_walk(struct tbl_walk *tbl_wlk)
{
	...
	if (!tbl_wlk->pasid_tbl_entry)
		pkvm_info("%02x:%02x.%x\t0x%016llx:0x%016llx\t0x%016llx:0x%016llx\t%-6d\t0x%016llx:0x%016llx:0x%016llx\n",
...
void root_tbl_walk(struct pkvm_iommu *iommu)
{
	...
	pkvm_info("IOMMU %d: Root Table Address: 0x%llx\n",
		 iommu->iommu.seq_id, (u64)iommu->pgt.root_pa);
	pkvm_info("B.D.F\tRoot_entry\t\t\t\tContext_entry\t\t\t\tPASID\tPASID_table_entry\n");
...
{
	pkvm_info("0x%013lx | 0x%016llx 0x%016llx 0x%016llx 0x%016llx 0x%016llx",
...
void domain_translation_struct_show(struct pkvm_iommu *iommu, u16 bdf, u32 pasid)
{
	...
	if (!(vreg->gsts & DMA_GSTS_TES)) {
		pkvm_info("DMA Remapping is not enabled on %s\n",
				   iommu->iommu.name);
		return;
	}
	...
	pkvm_info("Device %04x:%02x:%02x.%x ",
			   iommu->iommu.segment, bus, PCI_SLOT(devfn), PCI_FUNC(devfn));

	if (scalable)
		pkvm_info("with pasid %x @0x%llx\n", pasid, pgd);
	else
		pkvm_info("@0x%llx\n", pgd);

	if (!pgd) {
		pkvm_info("Device configured for pass through!\n");
	} else {
		pkvm_info("%-17s\t%-18s\t%-18s\t%-18s\t%-18s\t%-s\n",
