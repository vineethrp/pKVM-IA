...
/*
 * enable_pmu and enable_mediated_pmu live in pmu.c, which the hypervisor does
 * not build.  Define them here so the KVM sources shared with the hypervisor
 * (cpuid.c, vmx.c) still link.  enable_pmu keeps its KVM default and is
 * cleared by the pKVM hypervisor's hardware_setup(); the pKVM hypervisor never
 * supports a mediated PMU, so enable_mediated_pmu stays false.
 */
bool enable_pmu = true;
bool enable_mediated_pmu;
			  ...
			  union pkvm_hc_data *out)
{
	...
	case VCPU_REGS_RSP:
		...
		break;
	case VCPU_REG_RIP:
		out->cache_reg.rip = vcpu->arch.rip;
		break;
	case VCPU_REG_PDPTR:
		out->cache_reg.pdptrs[0] = vcpu->arch.pdptrs[0];
		out->cache_reg.pdptrs[1] = vcpu->arch.pdptrs[1];
		out->cache_reg.pdptrs[2] = vcpu->arch.pdptrs[2];
		out->cache_reg.pdptrs[3] = vcpu->arch.pdptrs[3];
		break;
	case VCPU_REG_CR0:
...
static void update_vcpu_state_from_host(struct kvm_vcpu *vcpu)
{
	...
	if (!pkvm_is_protected_vcpu(vcpu)) {
		...
		if (kvm_register_is_dirty(shared_vcpu, VCPU_REG_RIP))
			kvm_register_mark_dirty(vcpu, VCPU_REG_RIP);
		else
			shared_vcpu->arch.rip = kvm_rip_read(vcpu);
		/* Update the npVM's GPRs from the host */
		memcpy(vcpu->arch.regs, shared_vcpu->arch.regs,
		       NR_VCPU_GENERAL_PURPOSE_REGS * sizeof(*vcpu->arch.regs));
		/* RIP has its own storage, it is no longer part of regs[]. */
		vcpu->arch.rip = shared_vcpu->arch.rip;
		 ...
		 */
		kvm_rax_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_RAX]);
		kvm_rbx_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_RBX]);
		kvm_rcx_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_RCX]);
		kvm_rdx_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_RDX]);
		kvm_rsi_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_RSI]);
		kvm_rdi_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_RDI]);
		if (kvm_register_is_dirty(shared_vcpu, VCPU_REGS_RSP))
			kvm_rsp_write(vcpu, shared_vcpu->arch.regs[VCPU_REGS_RSP]);
		kvm_rbp_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_RBP]);
		kvm_r8_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_R8]);
		kvm_r9_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_R9]);
		kvm_r10_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_R10]);
		kvm_r11_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_R11]);
		kvm_r12_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_R12]);
		kvm_r13_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_R13]);
		kvm_r14_write_raw(vcpu, shared_vcpu->arch.regs[VCPU_REGS_R14]);

		/* Reserve R15 for pKVM for future extensions. */
		kvm_r15_write_raw(vcpu, 0);
		...
		if (!pkvm_vcpu_is_pvmfw_bsp(vcpu) &&
		    kvm_register_is_dirty(shared_vcpu, VCPU_REG_RIP))
			kvm_rip_write(vcpu, shared_vcpu->arch.rip);
	}
...
static void share_vcpu_state_with_host(struct kvm_vcpu *vcpu)
{
	...
	if (!pkvm_is_protected_vcpu(vcpu)) {
		...
		memcpy(shared_vcpu->arch.regs, vcpu->arch.regs,
		       NR_VCPU_GENERAL_PURPOSE_REGS * sizeof(*vcpu->arch.regs));
		/* RIP has its own storage, it is no longer part of regs[]. */
		shared_vcpu->arch.rip = vcpu->arch.rip;
		kvm_register_mark_available(shared_vcpu, VCPU_REGS_RSP);
		kvm_register_mark_available(shared_vcpu, VCPU_REG_RIP);
		shared_vcpu->arch.cr0 = kvm_read_cr0(vcpu);
		kvm_register_mark_available(shared_vcpu, VCPU_REG_CR0);
		shared_vcpu->arch.cr4 = kvm_read_cr4(vcpu);
		kvm_register_mark_available(shared_vcpu, VCPU_REG_CR4);
					   ...
					   union pkvm_hc_data *in, union pkvm_hc_data *out)
{
	...
	case __pkvm__flush_tlb_current:
		...
		break;
	case __pkvm__flush_tlb_gva:
		kvm_x86_call(flush_tlb_gva)(vcpu, pkvm_hc_input1(hvcpu), NULL);
		break;
	case __pkvm__flush_tlb_guest:
