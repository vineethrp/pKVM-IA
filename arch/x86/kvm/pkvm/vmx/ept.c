...
static void *host_ept_zalloc_page(struct pkvm_memcache *mc)
{
	...
	/* The host EPT is also the protected IOMMU passthrough domain. */
...
static void host_ept_pte_set(void *ptep, u64 spte)
{
	...
	/* Make host-EPT updates visible to non-coherent IOMMU page walkers. */
	if (!pkvm_iommu_paging_structure_coherency())
		clflush_cache_range(ptep, sizeof(spte));
}
