...
#define raw_spin_lock_irqsave(lock, ...) pkvm_spin_lock(lock)
#undef raw_spin_unlock_irqrestore
#define raw_spin_unlock_irqrestore(lock, ...) pkvm_spin_unlock(lock)
...
void qi_global_iec(struct intel_iommu *iommu)
{
	...
	if (pkvm_enabled()) {
		...
		if (ret)
			pr_warn("%s: iommu%d: IEC flush failed: %d\n",
...
#else
	/*
	 * pKVM never disables translation after initialization, and this helper
	 * must not be called before then.
	 */
