					 ...
					 u8 devfn, int alloc)
{
	...
	if (*entry & 1)
		...
		context = iommu_alloc_pages_node_sz(iommu->node, GFP_ATOMIC,
						    SZ_4K);
#else
		context = pkvm_iommu_ts_page(iommu);
#endif
		if (!context)
			return NULL;
	...
	while (1) {
		...
		if (!dma_pte_present(pte)) {
			...
			if (!tmp_page)
				return NULL;
#else
			struct pkvm_page_range page_range = pop_pkvm_memcache(&domain->mc,
									      hyp_phys_to_virt);
			tmp_page = hyp_phys_to_virt(page_range.addr);
			if (!tmp_page)
				return NULL;
...
#else
				push_pkvm_memcache(&domain->mc, tmp_page, PAGE_SIZE, hyp_virt_to_phys);
#endif
			}
			else
...
#ifdef __PKVM_HYP__
push_memcache:
	push_pkvm_memcache(&domain->mc, hyp_phys_to_virt(dma_pte_addr(parent_pte)),
			   PAGE_SIZE, hyp_virt_to_phys);
#endif
}
...
{
#ifdef __PKVM_HYP__
	unsigned nr_pages = domain->mc.count;
#endif
...
#else
	ret= pkvm_get_domain(pgd, did, info, IOMMU_NO_PASID);
	if (ret) {
		 ...
		 gfp_t gfp)
{
	...
	if (domain->iotlb_sync_map)
		cache_tag_flush_range_np(domain, iov_pfn << VTD_PAGE_SHIFT,
					 (iov_pfn + nr_pages - 1) << VTD_PAGE_SHIFT);
#endif
	return 0;
}
...
void domain_context_clear_one(struct device_domain_info *info, u8 bus, u8 devfn)
{
	...
	if (sm) {
		...
		pgd = __pkvm_va(context_get_address_root(context));
	}
#endif
	...
	if (sm)
		...
		pkvm_put_domain(pgd, did, info, IOMMU_NO_PASID);
#endif
}
...
static struct dmar_domain *paging_domain_alloc(struct device *dev, bool first_stage)
{
	...
	if (pkvm_enabled()) {
		int ret = pv_alloc_domain(info, domain);
				      ...
				      struct intel_iommu *iommu, u32 flags)
{
	...
	if (pkvm_enabled() && (flags & IOMMU_HWPT_ALLOC_DIRTY_TRACKING))
		return ERR_PTR(-EOPNOTSUPP);
...
static bool intel_iommu_capable(struct device *dev, enum iommu_cap cap)
{
	...
	case IOMMU_CAP_DIRTY_TRACKING:
		/* pKVM doesn't yet support it. */
		return !pkvm_enabled() && ssads_supported(info->iommu);
	default:
		return false;
	}
}
