...
#define spin_lock_irqsave(lock, ...) pkvm_spin_lock(lock)
#undef spin_unlock_irqrestore
#define spin_unlock_irqrestore(lock, ...) pkvm_spin_unlock(lock)
			     ...
			     ioasid_t pasid, enum cache_tag_type type)
{
	...
	if (type == CACHE_TAG_DEVTLB || type == CACHE_TAG_NESTING_DEVTLB) {
		...
		return tag->iommu == iommu && tag->bus == info->bus &&
